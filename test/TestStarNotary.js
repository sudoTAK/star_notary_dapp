...
it("can add the star name and star symbol properly", async () => {
  ...
  let tokenId = 42;
  let instance = await StarNotary.deployed();
  await instance.createStar("Star 13!", tokenId, { from: accounts[0] });
  //2. Call the name and symbol properties in your Smart Contract and compare with the name and symbol provided
  assert.equal(await instance.name.call(), "Star Certifier");

  assert.equal(await instance.symbol.call(), "SCT");
});
...
it("lets 2 users exchange stars", async () => {
  ...
  let tokenId1 = 123;
  let instance = await StarNotary.deployed();
  await instance.createStar("Star 123!", tokenId1, { from: accounts[0] });

  let tokenId2 = 321;
  await instance.createStar("Star 321!", tokenId2, { from: accounts[1] });
...
it("lets a user transfer a star", async () => {
  ...
  let tokenId = 45;
  let instance = await StarNotary.deployed();
  await instance.createStar("Star 45!", tokenId, { from: accounts[0] });
...
it("lookUptokenIdToStarInfo test", async () => {
  ...
  let tokenId = 666;
  let instance = await StarNotary.deployed();
  await instance.createStar("Star 666!", tokenId, { from: accounts[0] });
  assert.equal(
